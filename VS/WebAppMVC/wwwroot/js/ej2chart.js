function EJ2ChartExample(divId){
    var chart = new ej2.charts.Chart({
        ...
        tooltip: { enable: true },
        width: ej2.base.Browser.isDevice ? '100%' : '60%',
        //Initializing Chart Title
        title: 'Weather Condition JPN vs DEU',
        load: function (args) {
            var axisTheme = location.hash.split('/')[1];
            axisTheme = axisTheme ? axisTheme : 'Material';
            args.chart.theme = (axisTheme.charAt(0).toUpperCase() + axisTheme.slice(1));
        }
    });
    chart.appendTo(divId);
}
